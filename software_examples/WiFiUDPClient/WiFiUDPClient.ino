...
#define NETWORK_WAIT_MS 1                                 // task delay in polling loop
...
enum {
  ...
  STATE_NETWORK_INIT,  // setting up wifi
  STATE_NETWORK_WAIT,  // waiting for intial connect
...
void setup() {
  /* WiFI needs min 80MHz, don't need that much for LoRa */
#if WIFI_USED == 1
  setCpuFrequencyMhz(80E6);
#else
  setCpuFrequencyMhz(10E6);
#endif // WIFI_USED
...
void loop() {
  do {
    ...
    switch (curr_state) {
      ...
      case STATE_NETWORK_INIT:  //Connect to the network
#if WIFI_USED == 1
        connectToWiFi(networkName, networkPswd);
#else
        SPI.begin(RF96_CLK, RF96_MISO, RF96_MOSI, RF96_CS);
        LoRa.setPins(RF96_CS, -1, RF96_DIO0);
      ...
      case STATE_SEND:  //Send a packet
         ...
         */
          ...
          size_t length = snprintf(txBuffer, sizeof(txBuffer),
          ...
          LoRa.beginPacket();
          LoRa.write((uint8_t*)txBuffer, length);
          LoRa.endPacket();
  ...
  if (lora_active) // make sure to set transceiver into sleep in any case
    LoRa.sleep();
